...
def ZLabHookOut(text):
	zlabHookedPrint(text)
	return 0,1,text

def ZLabHookErr(text):
	zlabHookedPrint(text)
	return 0,1,text

phOut = PrintHook(1)
phOut.Start(ZLabHookOut)
phErr = PrintHook(0)
phErr.Start(ZLabHookErr)
print 'python stdout and stderr are now hooked.'
#phOut.Stop()
